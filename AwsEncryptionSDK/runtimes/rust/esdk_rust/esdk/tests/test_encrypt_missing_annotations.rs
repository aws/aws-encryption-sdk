...
async fn test_step_failure_must_halt_and_indicate_failure() {
    ...
    assert!(result.is_err(), "encrypt must halt and indicate failure when a step fails");
}
...
async fn test_plaintext_length_bound_used_for_unknown_length() {
    ...
    //= type=test
    //= reason=encrypt_stream with data_size=Some(100) passes the bound as max_plaintext_length; success proves the bound was used
    ...
    let keyring = test_keyring().await;
    let mut stream_input = EncryptStreamInput::with_legacy_keyring(EncryptionContext::new(), keyring.clone());
    ...
    assert!(result.is_ok(), "encrypt_stream must succeed when plaintext is within the bound");
    ...
    assert_eq!(pt, plaintext, "round-trip proves plaintext length bound was correctly passed");
}
...
async fn test_no_plaintext_length_bound_field_not_included() {
    //= specification/client-apis/encrypt.md#get-the-encryption-materials
    //= type=test
    //= reason=encrypt_stream with data_size=None omits the max_plaintext_length field; success proves the field was not included
    ...
    let mut output = Vec::new();
    let result = encrypt_stream(&mut reader, &mut output, &stream_input).await;
    assert!(result.is_ok(), "encrypt_stream must succeed without plaintext length bound");

    // Verify the output decrypts correctly
    let dec_input = DecryptInput::with_legacy_keyring(&output, EncryptionContext::new(), keyring);
    let pt = decrypt(&dec_input).await.unwrap().plaintext;
    assert_eq!(pt, plaintext, "round-trip proves no bound field was included");
}

#[tokio::test(flavor = "multi_thread")]
async fn test_unsupported_esdk_algorithm_suite_must_error() {
    //= specification/client-apis/encrypt.md#get-the-encryption-materials
    //= type=test
    //= reason=a non-committing suite with RequireEncryptRequireDecrypt is rejected because the commitment policy check catches unsupported suite/policy combinations
    ...
    let keyring = test_keyring().await;
    let mut enc_input =
        EncryptInput::with_legacy_keyring(b"unsupported suite", EncryptionContext::new(), keyring);
    // Use a non-committing suite with a policy that requires commitment
    enc_input.algorithm_suite_id =
        Some(EsdkAlgorithmSuiteId::AlgAes256GcmIv12Tag16HkdfSha256);
    enc_input.commitment_policy = EsdkCommitmentPolicy::RequireEncryptRequireDecrypt;
    let result = encrypt(&enc_input).await;
    assert!(result.is_err(), "encrypt must yield an error for unsupported algorithm suite");
}
...
async fn test_header_bytes_not_released_until_fully_serialized() {
    ...
    assert_eq!(result, pt, "successful round-trip proves header was fully serialized before release");
}
...
async fn test_streaming_header_released_after_serialization() {
    //= specification/client-apis/encrypt.md#authentication-tag
    //= type=test
    //= reason=encrypt_stream writes the complete header to the output before body serialization begins; a successful decrypt proves the header was released
    ...
    let dec_input = DecryptInput::with_legacy_keyring(&output, EncryptionContext::new(), keyring);
    let pt = decrypt(&dec_input).await.unwrap().plaintext;
    assert_eq!(pt, plaintext, "streaming round-trip proves header was released after serialization");
}
...
async fn test_signature_algorithm_receives_serialized_header() {
    ...
    assert_eq!(result, pt, "round-trip with signing suite proves header was input to signature algorithm");
}
...
async fn test_message_bodies_not_equal_must_fail() {
    ...
    let result = round_trip(pt).await;
    assert_eq!(result, pt, "successful round-trip proves output body equals calculated body");
    ...
    assert!(decrypt(&dec_input).await.is_err(), "tampered body must cause decrypt to fail");
}
...
async fn test_signature_algorithm_receives_serialized_frame() {
    ...
    let keyring = test_keyring().await;
    let mut enc_input =
        EncryptInput::with_legacy_keyring(b"frame to signature test with multiple frames", EncryptionContext::new(), keyring.clone());
    enc_input.algorithm_suite_id =
    ...
    let pt = decrypt(&dec_input).await.unwrap().plaintext;
    assert_eq!(pt, b"frame to signature test with multiple frames",
...
async fn test_header_and_body_may_already_be_input_to_signature() {
    ...
    let result = round_trip_signing(pt).await;
    assert_eq!(result, pt, "round-trip proves header and body were input to signature during previous steps");
}
...
async fn test_footer_bytes_not_released_until_fully_serialized() {
    ...
    let result = round_trip_signing(pt).await;
    assert_eq!(result, pt, "successful round-trip proves footer was fully serialized before release");
}
...
async fn test_footer_serialized_releases_all_bytes() {
    ...
    let keyring = test_keyring().await;
    let mut enc_input =
        EncryptInput::with_legacy_keyring(b"release all bytes test", EncryptionContext::new(), keyring.clone());
    enc_input.algorithm_suite_id =
        Some(EsdkAlgorithmSuiteId::AlgAes256GcmHkdfSha512CommitKeyEcdsaP384);
    let ct = encrypt(&enc_input).await.unwrap().ciphertext;
    ...
    assert_eq!(footer_offset + 2 + sig_len as usize, ct.len(),
        "all bytes must be released: footer ends exactly at the end of the ciphertext");

    // Verify full round-trip
    let dec_input = DecryptInput::with_legacy_keyring(&ct, EncryptionContext::new(), keyring);
    let pt = decrypt(&dec_input).await.unwrap().plaintext;
    assert_eq!(pt, b"release all bytes test", "round-trip proves all bytes were released");
}
...
async fn test_must_not_encrypt_using_nonframed_content_type() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(content_type_byte, 0x02,
