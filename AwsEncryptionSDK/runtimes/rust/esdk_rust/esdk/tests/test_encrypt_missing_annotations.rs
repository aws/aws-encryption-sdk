...
async fn test_step_failure_must_halt_and_indicate_failure() {
    ...
    assert!(
        result.is_err(),
        "encrypt must halt and indicate failure when a step fails"
    );
}
...
async fn test_unsupported_esdk_algorithm_suite_must_error() {
    ...
    assert!(
        result.is_err(),
        "encrypt must yield an error for unsupported algorithm suite"
    );
}
...
async fn test_message_bodies_not_equal_must_fail() {
    ...
    assert!(
        ...
        "tampered body must cause decrypt to fail"
    );
}
