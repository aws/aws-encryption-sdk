...
async fn test_encrypted_data_keys_serialization_order() {
    ...
    for version in VERSIONS {
        let ct =
            encrypt_with_version(b"edk serialization order", version, multi_keyring.clone()).await;
        let count_offset = edk_count_offset(&ct, version);
        let edk_count = u16::from_be_bytes([ct[count_offset], ct[count_offset + 1]]);
        assert_eq!(
            edk_count, 2,
            "{version:?}: multi-keyring must produce EDK count of 2"
        );
        ...
        assert_eq!(
            edk1_id, "child0 Namespace",
            "{version:?}: EDK 1 provider ID"
        );
        assert_eq!(
            edk2_id, "child1 Namespace",
            "{version:?}: EDK 2 provider ID"
        );
    }
}
...
async fn test_encrypted_data_key_count_is_2_bytes_uint16() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            edk_count, 1,
            "{version:?}: single keyring must produce exactly 1 EDK"
        );
...
async fn test_edk_entry_serialization_order() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            std::str::from_utf8(kp_id).unwrap(),
            "child0 Namespace",
            "{version:?}"
        );
...
async fn test_key_provider_id_length_matches_data() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            kp_id_len as usize,
            kp_id.len(),
            "{version:?}: Key Provider ID length must match data"
        );
    }
}
...
async fn test_key_provider_id_is_utf8() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            kp_id_str, "child0 Namespace",
            "{version:?}: Key Provider ID must match keyring namespace"
        );
    }
}
...
async fn test_key_provider_information_length_field_is_2_bytes_uint16() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            read_len, kp_info_len,
            "{version:?}: info length field must be 2-byte UInt16"
        );
        assert_eq!(kp_info_len as usize, kp_info.len(), "{version:?}");
    }
}
...
async fn test_key_provider_information_length_matches_data() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            ...
            kp_info.len(),
            "{version:?}: info length must match data"
        );
    }
}
...
async fn test_key_provider_information_is_bytes() {
    ...
    for version in VERSIONS {
        ...
        assert!(
            kp_info_len as usize >= key_name.len() + 4 + 4 + 12,
            "{version:?}: provider info too short for raw AES keyring format"
        );
        assert_eq!(
            &kp_info[..key_name.len()],
            &key_name[..],
            "{version:?}: provider info must start with key name"
        );
        let tag_len_bits = u32::from_be_bytes(
            kp_info[key_name.len()..key_name.len() + 4]
                .try_into()
                .unwrap(),
        );
        assert_eq!(
            tag_len_bits, 128,
            "{version:?}: tag length must be 128 bits"
        );
        let iv_len_bytes = u32::from_be_bytes(
            kp_info[key_name.len() + 4..key_name.len() + 8]
                .try_into()
                .unwrap(),
        );
...
async fn test_encrypted_data_key_length_field_is_2_bytes_uint16() {
    ...
    for version in VERSIONS {
        ...
        let first_edk_offset = edk_count_offset(&ct, version) + 2;
        let (kp_id_len, _, kp_info_len, _, edk_len, edk_data) =
            parse_edk_raw_at(&ct, first_edk_offset);
        ...
        assert_eq!(
            read_len, edk_len,
            "{version:?}: EDK length field must be 2-byte UInt16"
        );
        assert_eq!(edk_len as usize, edk_data.len(), "{version:?}");
    }
}
...
async fn test_encrypted_data_key_length_matches_data() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            edk_len as usize,
            edk_data.len(),
            "{version:?}: EDK length must match data"
        );
    }
}
...
async fn test_encrypted_data_key_is_bytes() {
    ...
    for version in VERSIONS {
        ...
        assert!(
            edk_len > 0,
            "{version:?}: Encrypted Data Key should be non-empty"
        );
        ...
        assert_eq!(
            copied.len(),
            edk_len as usize,
            "{version:?}: all EDK bytes must be readable"
        );
        assert!(
            copied.len() > 16,
            "{version:?}: EDK ciphertext must be longer than just a GCM tag"
        );
        // The ciphertext should not be all zeros (it's encrypted key material)
        assert!(
            copied.iter().any(|&b| b != 0),
            "{version:?}: EDK ciphertext must contain non-zero bytes"
        );
    }
}
