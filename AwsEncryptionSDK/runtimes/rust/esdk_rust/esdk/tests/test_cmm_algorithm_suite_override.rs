...
mod fixtures;
use aws_esdk::*;
use aws_mpl_legacy::dafny::operation::decrypt_materials::{DecryptMaterialsInput, DecryptMaterialsOutput};
use aws_mpl_legacy::dafny::operation::get_encryption_materials::{GetEncryptionMaterialsInput, GetEncryptionMaterialsOutput};
use aws_mpl_legacy::dafny::types::cryptographic_materials_manager::{CryptographicMaterialsManager, CryptographicMaterialsManagerRef};
...
impl CryptographicMaterialsManager for SuiteOverrideCmm {
    fn get_encryption_materials(&self, input: GetEncryptionMaterialsInput) -> Result<GetEncryptionMaterialsOutput, MplError> {
    ...
    fn decrypt_materials(&self, input: DecryptMaterialsInput) -> Result<DecryptMaterialsOutput, MplError> {
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
...
async fn test_encrypt_uses_cmm_suite_not_input_suite() {
    ...
    let keyring = mpl()
        ...
        .key_namespace(ns).key_name(name)
    ...
    let cmm = CryptographicMaterialsManagerRef::from(SuiteOverrideCmm {
        inner: mpl().create_default_cryptographic_materials_manager()
            .keyring(keyring.clone()).send().await.unwrap(),
    ...
    assert_eq!(out.algorithm_suite_id, SuiteId::AlgAes256GcmHkdfSha512CommitKey);
    assert_ne!(out.algorithm_suite_id, SuiteId::AlgAes256GcmHkdfSha512CommitKeyEcdsaP384);

    // Round-trip to prove the ciphertext is valid.
    let dec = decrypt(&DecryptInput::with_legacy_keyring(
        &out.ciphertext, EncryptionContext::new(), keyring,
    )).await.unwrap();
    assert_eq!(dec.plaintext, b"hello");
    assert_eq!(dec.algorithm_suite_id, SuiteId::AlgAes256GcmHkdfSha512CommitKey);
}
