...
async fn test_edk_count_greater_than_zero() {
    ...
    for version in VERSIONS {
        let ct = encrypt_with_version(b"count > 0", version, keyring.clone()).await;
        let parsed = parse_edk_section(&ct, version);
        //= specification/data-format/message-header.md#encrypted-data-key-count
        //= type=test
        ...
        assert!(
            parsed.edk_count > 0,
            "{version:?}: EDK count must be greater than 0"
        );
    }
}
...
async fn test_edk_count_zero_rejected_on_decrypt() {
    ...
    for version in VERSIONS {
        ...
        //# This value MUST be greater than 0.
        assert!(
            decrypt(&dec).await.is_err(),
            "{version:?}: decrypt must reject EDK count of 0"
        );
    }
}
...
async fn test_edk_count_max_enforcement_encrypt() {
    ...
    assert!(
        encrypt(&input).await.is_err(),
        "encrypt must fail when EDK count exceeds max"
    );
}
...
async fn test_edk_count_max_enforcement_decrypt() {
    ...
    assert!(
        decrypt(&dec).await.is_err(),
        "decrypt must fail when EDK count exceeds max"
    );
}
...
async fn test_key_provider_info_length_interpreted_as_uint16() {
    ...
    for version in VERSIONS {
        ...
        assert_eq!(
            ...
            "{version:?}: UInt16 provider info length must match actual provider info bytes"
        );
    }
}
