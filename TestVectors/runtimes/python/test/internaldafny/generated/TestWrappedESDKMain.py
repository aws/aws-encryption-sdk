...
class default__:
    ...
    @staticmethod
    def RunManifestTests():
        default__.TestGenerateEncryptManifest()
        default__.TestEncryptManifest()
    ...
    def TestGenerateEncryptManifest():
        ...
        d_1_result_: Wrappers.Result
        out1_: Wrappers.Result
        out1_ = WriteVectors.default__.WriteTestVectors(EsdkManifestOptions.ManifestOptions_EncryptManifest((d_0_directory_) + (_dafny.Seq("dafny/TestVectors/test/")), 5))
        d_1_result_ = out1_
        if (d_1_result_).is_Failure:
            _dafny.print(_dafny.string_of((d_1_result_).error))
        if not((d_1_result_).is_Success):
    ...
    def TestEncryptManifest():
        ...
        d_1_result_: Wrappers.Result
        out1_: Wrappers.Result
        out1_ = EsdkTestManifests.default__.StartEncryptVectors(EsdkManifestOptions.ManifestOptions_Encrypt((d_0_directory_) + (_dafny.Seq("dafny/TestVectors/test/")), _dafny.Seq("encrypt-manifest.json"), (d_0_directory_) + (_dafny.Seq("dafny/TestVectors/test/")), Wrappers.Option_None()))
        d_1_result_ = out1_
        if (d_1_result_).is_Failure:
            _dafny.print(_dafny.string_of((d_1_result_).error))
        if not((d_1_result_).is_Success):
