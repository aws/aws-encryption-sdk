...
async fn test_header_serialization_order() {
    for version in VERSIONS {
        ...
        // Verify header auth is immediately adjacent to header body end
        // by checking the auth tag bytes at the computed offset are valid.
        let tag_start = match version {
            Version::V1 => {
                // V1: IV (all zeros) then tag
                let iv = &ct[header_auth_offset..header_auth_offset + (header_auth_size - auth_tag_len)];
                assert!(
                    iv.iter().all(|&b| b == 0),
                    "{version:?}: V1 header auth IV at offset {header_auth_offset} must be all zeros"
                );
                header_auth_offset + (header_auth_size - auth_tag_len)
            }
            Version::V2 => header_auth_offset,
        };
        let tag = &ct[tag_start..tag_start + auth_tag_len];
        assert!(
            tag.iter().any(|&b| b != 0),
            "{version:?}: header auth tag at offset {tag_start} must be non-zero"
        );
    }
}
...
async fn test_nonframed_frame_length_must_be_zero() {
    ...
    for version in VERSIONS {
        ...
        // Confirm this is indeed nonframed content
        assert_eq!(
            ct[content_type_offset], 0x01,
            "{version:?}: external vector must be nonframed (content type 0x01)"
        );

        // Positive test: frame length MUST be 0 for nonframed content
        let frame_length = u32::from_be_bytes([
            ct[frame_length_offset],
            ct[frame_length_offset + 1],
            ct[frame_length_offset + 2],
        ...
        assert_eq!(
            frame_length, 0,
            "{version:?}: nonframed message frame length must be 0, got {frame_length}"
        );
    }
}

#[tokio::test(flavor = "multi_thread")]
async fn test_nonframed_nonzero_frame_length_rejected() {
    //= spec/data-format/message-header.md#frame-length
    //= type=test
    //= reason=Tampering the frame length to non-zero in a nonframed vector and verifying decrypt rejects it proves the constraint is enforced on read.
    //# When the [content type](#content-type) is nonframed, the value of this field MUST be 0.
    let keyring = test_keyring().await;
    for version in VERSIONS {
        let mut ct = external_nonframed_ct(version).to_vec();

        let frame_length_offset = match version {
            Version::V1 => parse_v1_trailing_offsets(&ct).3,
            Version::V2 => parse_header_offsets(&ct).2,
        };

        // Tamper: set frame length to non-zero
