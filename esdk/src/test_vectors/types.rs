...
/// A cryptographic key entry from the test-vector framework keys manifest.
///
/// See: <https://github.com/awslabs/aws-crypto-tools-test-vector-framework>
...
impl TestResults {
    fn record_failure(&mut self, label: &str, test: &EncryptTest, e: &anyhow::Error) {
        println!(
            "Failed Test{label} {} {} {} {e:?}",
            test.name,
