use super::types::{Bytes, EDKs, Edk, Key, KeyMap, RequiredKeys};
...
pub(crate) fn parse_key_v1(data: &JsonValue, name: &str) -> Result<Key> {
    ...
    for (key_str, value) in data.as_object().unwrap() {
        match key_str.as_str() {
            ...
            "bits" => key.bits = u32::try_from(value.as_u64().unwrap()).context("bits too large for u32")?,
...
pub(crate) fn parse_edk(data: &JsonValue) -> Result<Edk> {
    ...
    for (key_str, value) in data.as_object().unwrap() {
        match key_str.as_str() {
            ...
            "ciphertext" => edk.ciphertext = decode_base64(value.as_str().unwrap())?,
            "keyProviderInfo" => {
                edk.key_provider_info = decode_base64(value.as_str().unwrap())?;
            }
...
pub(crate) fn parse_edks(data: &JsonValue) -> Result<EDKs> {
    let mut edks = EDKs::default();
    for value in data.as_array().unwrap() {
        edks.push(parse_edk(value)?);
    }
    Ok(edks)
}
