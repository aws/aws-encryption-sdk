...
// ===== Optional Esdk client =====================================================
//
// `Esdk` is an optional convenience layer over the free `encrypt` / `decrypt` /
// `encrypt_stream` / `decrypt_stream` functions. It holds an `EsdkConfig` (the
// fields the spec defines as "client config" — commitment policy and max
// encrypted data keys) and forwards each call to the corresponding free
// function with those fields filled in.
//
