...
# Module: CreateRawAesKeyringSmokeTest
...
class default__:
    ...
    @staticmethod
    def EvaluationRule(x):
        output: _dafny.Seq = _dafny.Seq("")
        if not(ValidAesWrappingKeyPartition.default__.IsInPartition((x).wrappingKey)):
            output = _dafny.Seq("Invalid wrapping key length")
            return output
        if (((x).wrappingAlg) == (AwsCryptographyMaterialProvidersTypes.AesWrappingAlg_ALG__AES128__GCM__IV12__TAG16())) and (not(Aes128WrappingKeyPartition.default__.IsInPartition((x).wrappingKey))):
            output = _dafny.Seq("Invalid wrapping key length")
            return output
        elif (((x).wrappingAlg) == (AwsCryptographyMaterialProvidersTypes.AesWrappingAlg_ALG__AES192__GCM__IV12__TAG16())) and (not(Aes192WrappingKeyPartition.default__.IsInPartition((x).wrappingKey))):
            output = _dafny.Seq("Invalid wrapping key length")
            return output
        elif (((x).wrappingAlg) == (AwsCryptographyMaterialProvidersTypes.AesWrappingAlg_ALG__AES256__GCM__IV12__TAG16())) and (not(Aes256WrappingKeyPartition.default__.IsInPartition((x).wrappingKey))):
            output = _dafny.Seq("Invalid wrapping key length")
            return output
        if ((x).keyNamespace) == (_dafny.Seq("aws-kms")):
            output = _dafny.Seq("Invalid key namespace for raw AES keyring")
            return output
        output = _dafny.Seq("ok")
        return output

    @staticmethod
    def InputToJSON(x):
        return CreateRawAesKeyringInput.default__.ToJSON(x)
    ...
    @staticmethod
    def DomainRepresentativeValues():
        output: _dafny.Seq = _dafny.Seq({})
        out0_: _dafny.Seq
        out0_ = CreateRawAesKeyringInput.default__.GetValues()
        output = out0_
        return output
    ...
    def GenerateJSON():
        ...
        d_0_operationsJSON_: _dafny.Seq
        d_0_operationsJSON_ = _dafny.Seq([])
        d_1_inputValues_: _dafny.Seq
        out0_: _dafny.Seq
        out0_ = default__.DomainRepresentativeValues()
        d_1_inputValues_ = out0_
        hi0_ = len(d_1_inputValues_)
        for d_2_i_ in range(0, hi0_):
            d_3_input_: AwsCryptographyMaterialProvidersTypes.CreateRawAesKeyringInput
            d_3_input_ = (d_1_inputValues_)[d_2_i_]
            d_4_json_: JSON_Values.JSON
            out1_: JSON_Values.JSON
            out1_ = default__.JSONToWriteForOperationInputValue(d_3_input_)
            d_4_json_ = out1_
            d_0_operationsJSON_ = (d_0_operationsJSON_) + (_dafny.Seq([d_4_json_]))
        outputJSON = JSON_Values.JSON_Object(_dafny.Seq([(default__.operationName, JSON_Values.JSON_Array(d_0_operationsJSON_))]))
        return outputJSON

    @staticmethod
    def JSONToWriteForOperationInputValue(input):
        output: JSON_Values.JSON = JSON_Values.JSON.default()()
        d_0_outputValue_: _dafny.Seq
        out0_: _dafny.Seq
        out0_ = default__.EvaluationRule(input)
        d_0_outputValue_ = out0_
        d_1_inputJSON_: JSON_Values.JSON
        d_1_inputJSON_ = default__.InputToJSON(input)
        d_2_outputJSON_: JSON_Values.JSON
        d_2_outputJSON_ = default__.OutputToJSON(d_0_outputValue_)
        output = JSON_Values.JSON_Object(_dafny.Seq([(_dafny.Seq("input"), d_1_inputJSON_), (_dafny.Seq("output"), d_2_outputJSON_)]))
        return output

    @_dafny.classproperty
    def operationName(instance):
        return _dafny.Seq("CreateRawAesKeyringSmokeTest")
