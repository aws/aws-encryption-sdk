...
class default__:
    ...
    def IsInPartition(x):
        def lambda0_(forall_var_0_):
            ...
            return not (((0) <= (d_0_i_)) and ((d_0_i_) < (len(x)))) or ((ord((x)[d_0_i_])) <= (127))

        return ((len(x)) > (0)) and (_dafny.quantifier(_dafny.IntegerRange(0, len(x)), True, lambda0_))

    @staticmethod
    def RepresentativeValues():
        output: _dafny.Seq = _dafny.Seq({})
        output = _dafny.Seq([_dafny.Seq("abc")])
        return output
