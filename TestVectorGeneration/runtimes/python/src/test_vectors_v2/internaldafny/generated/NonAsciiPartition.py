...
class default__:
    ...
    def IsInPartition(x):
        def lambda0_(exists_var_0_):
            ...
            return ((((0) <= (d_0_i_)) and ((d_0_i_) < (len(x)))) and ((ord((x)[d_0_i_])) > (127))) and ((ord((x)[d_0_i_])) <= (65535))

        return ((len(x)) > (0)) and (_dafny.quantifier(_dafny.IntegerRange(0, len(x)), False, lambda0_))

    @staticmethod
    def RepresentativeValues():
        output: _dafny.Seq = _dafny.Seq({})
        d_0_value_: _dafny.Seq
        d_0_value_ = _dafny.Seq("Hëllø \uD83C\uDF0D — こんにちは世界 — مرحبا بالعالم — Здравствуй, мир!")
        output = _dafny.Seq([d_0_value_])
        return output
