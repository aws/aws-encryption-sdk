...
# Module: ValidRawAesKeyNamespacePartition
...
class default__:
    ...
    @staticmethod
    def IsInPartition(x):
        return (x) != (_dafny.Seq("aws-kms"))

    @staticmethod
    def FilterNonAwsKms(s):
        d_0___accumulator_ = _dafny.Seq([])
        while True:
            with _dafny.label():
                if (len(s)) == (0):
                    return (d_0___accumulator_) + (_dafny.Seq([]))
                elif ((s)[0]) == (_dafny.Seq("aws-kms")):
                    in0_ = _dafny.Seq((s)[1::])
                    s = in0_
    ...
    def RepresentativeValues():
        ...
        d_0_values_ = out0_
        output = default__.FilterNonAwsKms(d_0_values_)
        if (len(output)) == (0):
            output = _dafny.Seq([_dafny.Seq("default-namespace")])
        return output
