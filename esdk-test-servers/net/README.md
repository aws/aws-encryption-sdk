# ESDK TestServer — .NET Language_Server
...
each operation to the AWS Encryption SDK for .NET
(`AwsEncryptionSDK/runtimes/net` in this repository).
...
This directory hosts only the .NET Language_Server; it consumes the commons
...
Export these before any `make` target (the Makefile does not set them):
...
```bash
make run-server PORT=8097        # foreground
# or, orchestrated:
make start-server PORT=8097
make wait-for-server PORT=8097
make stop-server PORT=8097
make check                       # build + test (the CI gate)
```
...
`Model.cs` (shapes + CBOR mapping), `Wire.cs` (protocol), `KeyringFactory.cs`
(config→MPL mapping), `Handlers.cs` (delegation + client registry).
